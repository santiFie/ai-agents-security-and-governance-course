...
Que se agrego en esta adaptacion (no estaba en el libro):
1. Circuit breaker en el emisor de logs (mismo patron que
   labs/ch10-labA-ollama/logi_agent_lab10a.py, funcion `_emit`): sin esto,
   `cloud_logging.Client()` puede construirse sin error aun con credenciales
   ADC vencidas -la reautenticacion recien se descubre al llamar
   `log_struct()`, que entonces tarda ~60s en agotar el timeout del
   metadata server de GCP antes de tirar la excepcion. Sin breaker, CADA
   evento logueado (y este lab loguea varios por ataque simulado) paga ese
   hang de 60s en serie. Con el breaker: al primer fallo de
   `log_struct()`, se deshabilita `_cloud_logger` para el resto de la
   sesion (fallback silencioso a print(), igual que si nunca hubiera
   credenciales).
2. Un driver MINIMO que si ejercita el logging con una invocacion real:
   a. `simulate_prompt_injection_attack()` -determinista, sin LLM- reproduce
      ...
      logs en Capa 3 y Capa 7"): un mensaje entrante con una instruccion
      inyectada dispara (i) deteccion en Capa 1 (Foundation Models), (ii)
      una tool_call de exfiltracion en Capa 3 (Agent Frameworks) y (iii) una
      notificacion cross-agent en Capa 7 (Agent Ecosystem) -las 3 capas que
      MAESTRO exige mirar en conjunto para ver la cadena completa de un
      ataque agentico, no solo el punto de entrada.
   b. `maestro_observability_agent` -agente ADK real (qwen3.5:9b local)- que
      ejercita las mismas tools instrumentadas via tool-calling real, no
      solo llamadas directas en Python. Framing: igual que Logi-Agent (Lab
      10.A), este agente juega el rol de "agente legitimo procesando un
      mensaje y usando las herramientas que tiene disponibles" -no hace
      falta el parrafo LAB_CONTEXT de autorizacion porque no se le pide que
      reproduzca un comportamiento vulnerable sin cuestionarlo; el punto
      pedagogico de ESTE lab (que el ataque deja rastro en varias capas) ya
      se demuestra de forma determinista y confiable en (a) -el driver con
      LLM es una demostracion ADICIONAL de que el mismo logging tambien
      funciona detras de tool-calls reales del agente, no un reemplazo.
...
proyecto GCP para correr. Para el driver con LLM, Ollama corriendo con
qwen3.5:9b ya descargado (ollama pull qwen3.5:9b).

Modo de verificacion sin LLM (logging estructurado + circuit breaker +
simulacion determinista del ataque cross-layer, sin tocar Ollama):
...
def log_maestro_event(layer: MAESTROLayer, event_type: str,
    ...
    Circuit breaker (agregado en esta adaptacion, no esta en el original de
    ch02-labs.md -mismo patron que labs/ch10-labA-ollama/logi_agent_lab10a.py):
    si `_cloud_logger.log_struct()` falla una vez (p.ej. ADC vencidas, que
    tardan ~60s en descubrirse), se deshabilita `_cloud_logger` para el
    resto del proceso en vez de reintentar el timeout en cada evento
    subsiguiente."""
...
# ── Agente ADK real (qwen3.5:9b local) que ejercita las mismas tools ─────
# num_ctx=8192, temperature=0.2 -- mismo patron verificado que Lab 10.A/8.1.
# reasoning_effort="low", NO "none" (cambiado en esta pasada -- ver hallazgo
# en el .md del lab, seccion "reasoning_effort: none vs low"): con "none" el
# driver ADK, en corridas reales del 2026-08-02 y 2026-08-05, saltaba directo
# de la primera function_call (process_inbound_message) al reporte final SIN
# pedir las otras dos tool calls, y aun asi el reporte de texto libre
# afirmaba haberlas ejecutado -una alucinacion reproducible, no un evento
# aislado. Con "low" el modelo intercala un paso de texto de planificacion
# antes de cada accion y termina llamando las 3 tools de verdad, con un
# reporte final que coincide con _EMITTED_LOGS. Trade-off real, no gratis:
# "low" hace que el ataque simulado se ejecute de punta a punta (exfiltra de
# verdad) en vez de fallar en silencio -mas honesto, pero tambien mas
# "exitoso" como ataque. Si "low" reintroduce el loop de narracion sin cerrar
# reporte que motivo originalmente "none" (ver Lab 2.1/2.2), evaluar `medium`
# como punto intermedio antes de volver a "none".
...
maestro_observability_agent = Agent(
    ...
    model=LiteLlm(**OLLAMA_MODEL_CONFIG),
    instruction="""
...
# ── --selftest: logging + circuit breaker + ataque simulado, SIN Ollama ──
def _self_test() -> None:
    ...
    # circuit breaker se active (confirmado en una corrida real: 58.6s de
    # wall time, contra un presupuesto de --selftest que deberia ser
    # cuestion de segundos). El circuit breaker en si ya se prueba mas abajo
    # con `_FlakyCloudLogger` (sin red real) -asi que aca se deshabilita
    # `_cloud_logger` de entrada para que --selftest no pague ese costo
    # tambien; el costo real de la primera llamada en una corrida normal
    # (no --selftest) sigue existiendo y sigue documentado en el .md.
    global _cloud_logger
    ...
    try:
        ...
        assert isinstance(maestro_observability_agent.model, LiteLlm)
        assert maestro_observability_agent.model.model == "ollama_chat/qwen3.5:9b"
        assert len(maestro_observability_agent.tools) == 3
        print("[selftest] OK: agente instanciado con LiteLlm(ollama_chat/qwen3.5:9b) y 3 tools.")

        print("\n[selftest] OK: Lab 2.3 verificado de punta a punta sin invocar Ollama.")
    finally:
